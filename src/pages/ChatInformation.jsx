...
const ChatInformation = () => {
  ...
  return selectedChat ? (
    <div className="absolute top-0 left-0 w-full h-screen overflow-x-hidden overflow-y-auto bg-zinc-100">
      ...
      <div className="flex flex-col justify-center items-center pt-10">
        ...
        <h1 className="text-[1.5rem] md:text-[1.75rem] lg:text-[2rem] mt-3 font-semibold tracking-tighter leading-none">
          {selectedChat?.isGroupChat
            ? selectedChat?.chatName
            : oppositeUser?.fullName}
        </h1>
        <h3 className="text-[1rem] md:text-[1.25rem] font-medium text-zinc-600 tracking-tighter">
          {!selectedChat.isGroupChat
            ? oppositeUser?.email
            : `Group : ${selectedChat?.users.length} members`}
        </h3>
      </div>
      <div className="w-full flex flex-col gap-2 mt-3 border-y border-zinc-400 px-2 md:px-4 py-4">
